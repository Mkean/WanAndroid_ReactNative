...
export default class HttpConfig {
  ...
  constructor(serverTag = HttpConst.ServerTag) {
    ...
    this.contentType = HttpConst.CONTENT_TYPE_URLENCODED;
  ...
  listenNetwork(NetInfo) {
    NetInfo.addEventListener(state => {
      CommonConstant.networkAvailable = Platform.OS === 'android' ? !!state.isInternetReachable : !!state.isConnected
      CommonConstant.networkInfo = state
    })
  }
}
